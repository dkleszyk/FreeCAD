...
void DrawViewSection::Restore(Base::XMLReader &reader)
{
    ...
    // Before v1.2 a fudge factor was added to the section line length
    // when rendering the line in QGIViewPart. Starting in v1.2 the fudge factor
    // is no longer applied. But then we need to update the SectionLineStretch
    // value from older versions to keep the rendered length consistent
    if (Base::getVersion(reader.ProgramVersion) < Base::Version::v1_2) {
        double preferredStretch = preferredSectionLineStretch();
