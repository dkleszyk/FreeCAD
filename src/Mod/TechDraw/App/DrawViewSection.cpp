    ...
    , m_shapeSize(0.0)
{
    ...
    // new v120
    // Was previously a fixed "fudge" factor in QGIViewPart::drawSectionLine.
    // Now an instance property here so that the section line can be controlled
    // without messing with font sizes.
    ADD_PROPERTY_TYPE(SectionLineAdditionalLength,
                      (2 * Preferences::dimFontSizeMM()),
                      agroup,
                      App::Prop_None,
                      "Length added to the section line on top of the value computed by SectionLineStretch.");
...
void DrawViewSection::onChanged(const App::Property* prop)
{
    ...
    if (prop == &SectionLineStretch || prop == &SectionLineAdditionalLength) {
